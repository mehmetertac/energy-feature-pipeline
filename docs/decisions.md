...
| ADR-007 | 2026-05-25 | NWP ingestion uses **GFS 0.5°** via **Herbie** (`0.5-degree`; older Herbie used `pgrb2.0p25`); default leads **6 / 12 / 24 / 48 h**; point extraction at **52.5°N, 13.4°E** (same as ERA5). | Open NOAA data on AWS; no extra credentials; aligns forecast vs reanalysis ablations on one grid point. ECMWF/ICON deferred until a second product is needed. |
